namespace SharpGame;

[GlobalClass]
public partial class WallSlide : State
{
	[Export]
	public float Acceleration = 6.0f;

	[Export]
	public float SlideSpeed = 75.0f;

	[Export]
	public float SnapLength = 8.0f;

	[Export]
	State _fall;

	[Export]
	State _idle;

	[Export]
	State _walk;

	PhysicsDirectSpaceState2D _spaceState = null;
	Vector2 _wallNormal = Vector2.Zero;
	bool _jumpOff = false;

	public override void Enter()
	{
		_spaceState ??= PhysicsServer2D.SpaceGetDirectState(CharacterBody2D.GetWorld2D().Space);
		_wallNormal = CharacterBody2D.GetWallNormal();

		_jumpOff = false;
	}

	public override State PhysicsProcess(double delta)
	{
		Vector2 velocity = CharacterBody2D.Velocity;

		velocity.X = 0.0f;
		velocity.Y = Mathf.Lerp(velocity.Y, SlideSpeed, Acceleration * (float)delta);

		CharacterBody2D.Velocity = velocity;

		if (_jumpOff)
			CharacterBody2D.Velocity = new Vector2(125.0f * _wallNormal.X, -125.0f);

		CharacterBody2D.MoveAndSlide();

		if (CharacterBody2D.IsOnFloor())
		{
			float input = Godot.Input.GetAxis("move_left", "move_right");

			if (!Mathf.IsZeroApprox(input))
				return _walk;

			return _idle;
		}

		bool isColliding =
			IsColliding(CharacterBody2D.Position, CharacterBody2D.Position + -_wallNormal * SnapLength);

		if (!isColliding)
			return _fall;

		return null;
	}

	public override State Input(InputEvent @event)
	{
		if (@event.IsAction("jump") && @event.IsPressed())
			_jumpOff = true;

		return null;
	}

	bool IsColliding(Vector2 from, Vector2 to)
	{
		PhysicsRayQueryParameters2D parameters = new()
		{
			From = from,
			To = to
		};
		parameters.Exclude.Add(CharacterBody2D.GetRid());

		return _spaceState.IntersectRay(parameters).Count != 0;
	}
}
